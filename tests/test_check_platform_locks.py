...
def package(name: str, version: str, registry: str, wheels: list[str], deps: list[str] = ()) -> str:
...
wheels = [
{chr(10).join("    " + w + "," for w in wheels)}
]
'''


def dep(name: str, version: str | None = None, registry: str | None = None, marker: str | None = None) -> str:
    parts = [f'name = "{name}"']
    if version:
