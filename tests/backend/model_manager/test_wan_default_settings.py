...
class TestWanDefaultSettings:
    def test_a14b_defaults(self) -> None:
        s = resolve_default_settings(BaseModelType.Wan, WanVariantType.T2V_A14B)
    ...
    def test_ti2v_5b_defaults(self) -> None:
        s = resolve_default_settings(BaseModelType.Wan, WanVariantType.TI2V_5B)
        assert s is not None
        assert s.steps == 30
        assert s.cfg_scale == 5.0

    def test_no_variant_falls_back_to_a14b_settings(self) -> None:
        s = resolve_default_settings(BaseModelType.Wan)
        assert s is not None
        assert s.steps == 40
