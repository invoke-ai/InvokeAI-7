...
class WanModelLoaderInvocation(BaseInvocation):
    ...
    def invoke(self, context: InvocationContext) -> WanModelLoaderOutput:
        ...
        # expert. For A14B, the user wires the other expert to
        # transformer_low_noise_model. The explicit slots define the roles;
        # filename-derived expert tags are advisory only. boundary_ratio falls back
        # to 0.875 unless a Diffusers component_source provides a recorded value.
        ...
        if main_is_diffusers:
            ...
            if self.transformer_low_noise_model is not None and main_variant != WanVariantType.TI2V_5B:
                ...
                if primary_expert == "none" and low_expert == "none":
                    ...
                    context.logger.warning(
                        "The Wan A14B single-file filename tags disagree with the explicit transformer wiring. "
