...
class PiDUpscaleInvocation(BaseInvocation, WithMetadata, WithBoard):
    ...
    def invoke(self, context: InvocationContext) -> ImageOutput:
        # Fail fast if the connected decoder is for a different backbone. PiD upscale runs the FLUX
        # backbone, so a FLUX PiD decoder is required (the base-agnostic loader accepts any decoder).
        assert_pid_decoder_matches_base(
            context.models.get_config(self.pid_decoder.decoder).base,
            BaseModelType.Flux,
            node_title="PiD Upscale",
        )
