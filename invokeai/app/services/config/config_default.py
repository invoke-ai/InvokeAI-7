...
class InvokeAIAppConfig(BaseSettings):
    ...
    auto_tiled_decode:             bool = Field(default=True,               description="Decode large images in tiles when an untiled decode's estimated working memory would take most of the GPU's memory (FLUX.1, Z-Image, Qwen-Image, Krea-2, Anima and Wan decodes). Turn off to decode untiled unless tiling is requested; the FLUX.1, Z-Image and Anima decodes then still retry tiled after running out of memory.")
