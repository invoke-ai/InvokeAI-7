...
class ImageIndexService(ImageIndexServiceBase):
    ...
    def _announce_backfill_start(self) -> bool:
        """Log that items without an embedding are being indexed. False if it could not.
        ...
        self._invoker.services.logger.info(f"Image index: indexing {status.pending} item(s){named}")
        return True
    ...
    def _report_backfill_outcome(self) -> None:
        ...
        every eligible item has an embedding, or the rest exhausted their attempts and are
        ...
        cumulative, so reporting it whole would warn about items that retired hours ago on
        ...
        if retired > 0:
            logger.warning(
                f"Image index: indexing finished with {retired} item(s) that could not be embedded "
                f"({status.embedded} of {status.total} indexed)"
            )
        else:
            logger.info(f"Image index: indexing complete ({status.embedded} of {status.total} items indexed)")
