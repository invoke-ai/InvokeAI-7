 ...
 *
 * It also checks the node types and edge fields each base emits against a committed fixture. That
  ...
  it('emits the node types and fields the backend has to provide', async () => {
    ...
    const contract = {
      _comment:
        ...
        'tests/app/invocations/test_frontend_graph_node_types.py, which checks every node type and ' +
        'field below against the backend invocation registry.',
      byBase,
