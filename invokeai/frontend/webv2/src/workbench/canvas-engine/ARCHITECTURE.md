...
`document/documentIndex.ts` builds one index per `stacks` identity (parent, path, sibling index, document order, and ancestor-effective enabled/locked/hidden per node). Selection and bbox edits keep `stacks` and cost no index. A value edit (transform, opacity, blend, name, source, config, or a flag) rewrites the edited nodes and their ancestors through structural sharing, so `stacks` changes; the reducer then registers a derived index for the new forests (`deriveIndexForValueEdit`) that extends the previous one: `byId` is a layered map that reads through to the previous index for every untouched id, `nodes` and `leaves` materialize only when a consumer reads them (and `leaves` keeps its identity when no leaf object changed), and the only entries constructed are the edited path plus the subtree under a group whose flags changed. After eight derivations the chain flattens onto its plain root: one override layer holding the newest entry of every id the chain touched, still pointing consumers at the root's cached compile so the flattening step recompiles only what the chain changed; only when overrides cover more than a quarter of the root is a plain index rebuilt. Lookups therefore read through at most nine layers, and nothing older than the root is retained. `documentModel.budget.test.ts` pins this at 10,000 nodes: a leaf edit constructs `depth + 1` entries, compiles `depth + 1` semantic nodes (ancestors keep their counts without walking their subtrees) and one leaf, and answers `lookupDocumentLeaf`/`lookupDocumentNodeState` without materializing the node list.

`document-model/documentModel.ts` compiles `SemanticLeaf`s and `SemanticNode`s from that index: `contributionEnabled`, `effectiveLocked`, and `documentHidden` fold in every ancestor, and a leaf or node keeps its identity while its contract object, place, and ancestor-effective state are unchanged. Every consumer that decides visibility, contribution, editability, export, or what a panel row shows reads these (or the `isLeaf*` predicates) rather than a node's own flags; `model.refusalFor(command)` is the single eligibility authority for enabling a command before it runs; for `reparent`, `group`, `ungroup` and `remove` it runs the same validator `prepare` uses without materializing an edit (`editsMaterialized` stays at zero), so a drag hover or a toolbar render never builds an inverse or postconditions. Locks are all-or-nothing over subtrees: no command edits, moves, groups, ungroups, or inserts into a subtree that holds a locked node or sits under one, while name, enablement, and lock flips stay open so a locked node can be unlocked.
...
`widgets/layers` consumes the seam and owns nothing about the document. `layerTreeRows.ts` turns `compileDocumentNodes` plus per-project panel state (expansion, a name filter) into `LayerTreeRow`s keyed on the forests, so a selection or bbox change rebuilds no row, and a row keeps its identity while its node, place, effective state, sibling position and expansion are unchanged; `layerPanelRows.ts` flattens the four stacks into one list with a header per non-empty stack. `LayersTree.tsx` virtualizes that list (fixed row heights per density, the focused item always mounted, each slot drawing the row its key names so a list change never relabels a mounted node) and is one WAI-ARIA tree: stack headers are level-one tree items that expand or collapse their stack, rows are tree items beneath them, every control inside an item is pointer-only (`tabIndex={-1}`) and hands focus back to its item when pressed, and the whole tree is a single tab stop with a roving focus that survives the focused item unmounting; the selection toolbar above it is likewise one tab stop with arrow keys between its buttons. Keyboard: arrows, Home/End, Left to the parent or header, Right into a stack or group, Enter/Space to select (with Shift/Ctrl), Alt+arrows to reorder and reparent (carrying the selection when the focused row is part of it, as a drag would), F2 to rename (a rename committed by clicking elsewhere leaves focus where the click put it), Shift+F10 for the row or stack menu, which carries every pointer action including hide-on-canvas. The tree holds the single drag context (pointer-within collision scoped to the stack, deterministic edge auto-scroll, hover-expand, indicator projected in linear time and checked with `model.refusalFor` so a locked, cyclic or too-deep landing shows its refusal inline), the single menu/properties host addressed by node id, and every store subscription and commit; a row receives one view model and one stable command handle, and a primary that changes outside the panel is revealed once: its collapsed stack and groups open and it is scrolled into view, after which the panel's scroll position is its own. Above 2,000 nodes the panel degrades explicitly: thumbnails and drag reordering switch off, every command stays. `layersPanel.budget.test.ts` pins the 2,000-node facts (exact row counts, one node visit per build, row identity across selection and a local expand, linear drop projection) and `LayersTree.browser.test.tsx` pins the mounted-row window, per-row commit counts (`layerPanelDiagnostics`) for selection and scroll, thumbnail requests, drag validation counts, the tab stop, degraded mode on a real 2,001-node document, external reveal, edge auto-scroll, and the keyboard journeys in Chromium; `scripts/run-accessibility-journeys.mjs` runs Axe over the panel on the representative project.

### Properties and Transform

Tool and operation settings live off the surface, as editor panes of the Layers panel (`widgets/layers/panes`). The Properties pane renders the running operation first and then the active tool, each as a `ToolPropertyForm` / `OperationPropertyForm` (`widgets/canvas/tool-presentation`): named, collapsible groups of typed rows (an optional preview card on top), with ONE sticky footer pinned to the pane's bottom edge — the running operation's verbs (status chip, Process/Reset, Apply with its save-as menu, Cancel), else the tool form's (the transform session's Apply/Cancel). Group ids are global so tools sharing a group's component keep its DOM and collapse state across the switch. The Transform pane edits the selected layer's transform without a tool — each field commits one undoable `patch`, and while the Transform tool holds a session the same fields drive its preview and Apply / Cancel land or drop it. Forms read the engine's option stores and document transactions directly: no state mirror. Staging and generation lock the tool groups in place (`inert`), the footer honors the same lock itself (it sits outside the sections), and an operation keeps its own controls live with Cancel always reachable. The panes are fixed parts of the panel — `LayerEditorPanes` owns their tabs, preferred height and collapse, persisted through the Layers widget's project state — not movable widgets. The same blocks host the Color/Swatches workspace (the project foreground/background pair), the History pane (the engine's labeled undo/redo stack via `history.getEntries()`/`stepBy`), and the Overview navigator (`previews.drawDocumentOverview` plus the live viewport handle).
...
PSD export is a planner/executor transaction over immutable snapshots. `PsdExportController` captures one `CanvasDocumentSnapshot`, walks the raster tree into planner inputs (groups as pass-through folders, exportable leaves with their captured content rects), then obtains a `CanvasRasterSnapshot` including disabled layers. `planPsdExport` reverses every level of the top-first tree into ag-psd's bottom-to-top order, writes each node's own visibility flag (a disabled group is a hidden folder), and flattens the merged preview from the leaves that contribute (enabled with every ancestor enabled), which is what Photoshop renders for the same flags. Folders whose subtree exports nothing are dropped. Execution reads only the detached surfaces; it cannot mix live layer caches from different document generations.
