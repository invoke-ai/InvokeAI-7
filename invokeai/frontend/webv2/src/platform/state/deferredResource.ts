...
/** Single-flight handle for a value that is fetched the first time it is wanted. */
export interface DeferredResource<T> {
  getStatus: () => DeferredResourceStatus;
  /** Starts the load if needed and resolves with the value; repeat callers share one promise. */
  load: () => Promise<T>;
  /** Starts the load and ignores the outcome; for warming a value before it is needed. */
  preload: () => void;
  /** Retries a failed load; a load that has not failed is returned as-is. */
  retry: () => Promise<T>;
}
 ...
 * cost of switching layout, and as the whole cost of opening a dialog, long
 * after the chunk had finished downloading.
 */
...
/**
 * Creates a resource around `loader`, running it at most once per successful
 * load and exposing the settled state React's `use()` reads.
 *
 * `validate` runs on the loaded value and may throw to reject the load; use it
 * where the caller has a shape contract the loader cannot express, such as a
 * dynamically imported module that must export a component.
 */
export const createDeferredResource = <T>(
  ...
  return {
    getStatus: (): DeferredResourceStatus => state.status,
    load,
    preload: () => {
