...
export const isMiniMaxH3CheckpointMainModelConfig = (config: AnyModelConfigWithExternal): config is MainModelConfig => {
 ...
 *  are still reachable.
 *
 *  It hides MiniMax H3 single-file transformer overrides, plus Wan A14B low-noise
 *  experts only when the user has a partner to
...
export const selectPrimaryMainModelOptions = <T extends AnyModelConfigWithExternal>(configs: T[]): T[] => {
  ...
  return configs.filter(
    (config) => !isMiniMaxH3CheckpointMainModelConfig(config) && (!isLowExpert(config) || !hasPartner(config))
  );
};
