...
- Run focused Python tests with `uv run --no-sync pytest <test-paths>`; the full CI suite is `uv run --no-sync pytest`. Slow tests are excluded by default; hardware validation is separate.
