...
3. **Define Default Settings**
   ...
   These live in the architecture's own declaration, keyed by variant. `None` is the fallback for
   a model whose variant is not listed.
   ...
   register(
       ...
       DefaultSettingsFacet(
           {
               NewModelVariantType.Turbo: MainModelDefaultSettings(steps=4, cfg_scale=1.0, ...),
               None: MainModelDefaultSettings(steps=20, cfg_scale=7.0, width=1024, height=1024),
           }
       ),
   )
   ```
...
- [ ] Create `architectures/defs/<base>.py` with a `register(...)` call — `validate()` refuses to boot without it
- [ ] Declare default settings in its `DefaultSettingsFacet`
:::
...
To allow users to easily download your model from the Model Manager UI, add it to the starter models catalogue.

The catalogue is a package, one module per architecture. Define the models in your own module —
`types.py` holds `StarterModel` itself, and `common.py` holds the encoders and upscalers that
several architectures share, so import an existing dependency from there rather than declaring a
second copy of it.
...
Then list them in the package's `__init__.py`. `STARTER_MODELS` is a curated order — it is what the
install dialog shows, in the sequence someone chose, and nothing could reconstruct that order if it
were derived — so insert where the models belong rather than appending.
...
- [ ] Define text encoder StarterModel if separate, or reuse one from `starter_models/common.py`
- [ ] Define quantized variants (FP8, GGUF, etc.)
- [ ] Set dependencies correctly
- [ ] Import them in `starter_models/__init__.py` and place them in `STARTER_MODELS`
:::
...
- invokeai
  - app/invocations
    - metadata.py
    - `[newmodel]`
      - `__init__.py` required, or none of this package's nodes are discovered
  ...
  - backend
    - architectures/defs
      - `[newmodel].py` the architecture's declaration; `validate()` refuses to boot without it
    - model_manager
      - taxonomy.py
      - configs
        - main.py
