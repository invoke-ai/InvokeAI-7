...
description: A step-by-step checklist for integrating a new foundational model type into InvokeAI, from backend model manager to frontend UI.
...
:::tip[Start here]
Run the scaffolder first — it creates the files below that can be generated and prints the ones that
cannot:
...
Most per-architecture facts live in **one file**, `invokeai/backend/architectures/defs/[newmodel].py`,
rather than scattered across core modules:

```python title="invokeai/backend/architectures/defs/newmodel.py"
register(
    BaseModelType.NewModel,
    LatentSpaceFacet(FLUX_16),                       # required: preview projection + spatial scale
    ConditioningFacet(NewModelConditioningInfo),     # required: what the text encoder produces
    VariantFacet({ModelType.Main: NewModelVariantType}),   # optional
    LoaderFlagsFacet(supports_fp8_storage=False),          # optional
)
```

Add one line to `invokeai/backend/architectures/__init__.py` to import it, and the registration runs.

This matters because of *when* mistakes surface. Before the registry, the facts below were spread
over core files and none of them failed at boot — a missing entry raised `Unsupported base model` on
the first preview, or broke conditioning deserialization mid-graph, on the first generation that
happened to use the new architecture. Now `validate()` refuses to start the app while a required
facet is missing, and names the file to edit.
...
| Variant enum | four hand-widened unions | `VariantFacet`, with all four CI-checked |
| fp8 storage exceptions | `config.base ==` inside the generic loader | `LoaderFlagsFacet` |

Reusing an existing `LatentSpace` is the normal case, not a shortcut: eleven of the fifteen shipped
architectures introduce no new preview data at all.

:::tip[Checklist: Architecture Registry]{icon="approve-check"}
- [ ] Create `invokeai/backend/architectures/defs/[newmodel].py`
- [ ] Declare `LatentSpaceFacet` — reuse an existing `LatentSpace` unless the VAE genuinely differs
- [ ] Declare `ConditioningFacet`
- [ ] Declare `VariantFacet` / `LoaderFlagsFacet` / `UNetDownscaleFacet` only if they apply
- [ ] Add the import line to `invokeai/backend/architectures/__init__.py`
- [ ] Run `pytest tests/backend/architectures` — it names anything still missing
:::
...
1. **Add `BaseModelType`**
   ...
   Declare your new model in the base model taxonomy.
...
3. **Define Default Settings**
   ...
   ```python title="invokeai/backend/model_manager/configs/main.py" ins={9-10}
   class MainModelDefaultSettings:
       ...
       def from_base(base: BaseModelType, variant: AnyVariant | None = None):
           match base:
               ...
               case BaseModelType.NewModel:  # NEW
                   return MainModelDefaultSettings(steps=20, cfg_scale=7.0, ...)
   ```
</Steps>

:::tip[Checklist: Model Manager]{icon="approve-check"}
...
5. **Update `AnyModelConfig` Union**
   ...
   ```python title="invokeai/backend/model_manager/configs/factory.py" ins={3-5}
...
Loaders convert the files on disk (described by the config) into PyTorch models in memory.
...
:::tip[Checklist: Model Loader]{icon="approve-check"}
...
:::tip[Checklist: Sampling and Denoise]{icon="approve-check"}
...
2. **Register Graph Builder**
   ...
   ```typescript title="invokeai/frontend/web/src/features/queue/hooks/useEnqueueCanvas.ts" ins={13-14}
   switch (base) {
     case 'sd-1':
     case 'sd-2':
     case 'sdxl':
...
3. **Update Type Definitions**
   ...
   ```typescript title="invokeai/frontend/web/src/features/nodes/util/graph/types.ts"
   // Add node types:
...
4. **Update Generation Mode Utilities**
   ...
   Update `addImageToImage.ts`, `addInpaint.ts`, and `addOutpaint.ts` to recognize your denoise node type.

   ```typescript title="invokeai/frontend/web/src/features/nodes/util/graph/generation/addImageToImage.ts"
...
:::tip[Checklist: State Management]{icon="approve-check"}
...
Ensure users can extract parameters from previously generated images.
...
:::tip[Checklist: Parameter Recall]{icon="approve-check"}
...
1. **Add Generation Modes**
   ...
   ```python title="invokeai/app/invocations/metadata.py" ins={11-14}
...
:::tip[Checklist: Metadata and Generation Modes]{icon="approve-check"}
...
To allow users to easily download your model from the Model Manager UI, add it to the starter models
catalog. Entries live one module per architecture; only the display order and the bundles are
central.

```python title="invokeai/backend/model_manager/starter_models/newmodel.py"
# Main Model
...
Components shared with other architectures — text encoders, and VAEs more than one architecture
installs — belong in `starter_models/common.py`, not in your module. Import them from there.
`dependencies` holds object references rather than ids, so a name that does not exist is an
`ImportError` at startup instead of a broken install later.

Then wire it into the catalog:

```python title="invokeai/backend/model_manager/starter_models/__init__.py"
from invokeai.backend.model_manager.starter_models.newmodel import (
    newmodel_encoder,
    newmodel_fp8,
    newmodel_main,
    newmodel_vae,
...
:::caution[The order of `STARTER_MODELS` is what users see]
The frontend does not sort this list. Its order is curated — it follows neither base nor model type
— so put your entries where they should appear, not simply at the end.
:::

:::tip[Checklist: Starter Models]{icon="approve-check"}
- [ ] Create `starter_models/[newmodel].py`
- [ ] Define main model StarterModel
- [ ] Define VAE StarterModel if separate
- [ ] Define text encoder StarterModel if separate — in `common.py` if other architectures use it too
- [ ] Define quantized variants (FP8, GGUF, etc.)
- [ ] Set dependencies correctly
- [ ] Import the module in `starter_models/__init__.py` and place the entries in `STARTER_MODELS`
- [ ] Add a `STARTER_BUNDLES` entry if the architecture ships a launchpad bundle
:::
...
**Backend Config** — `invokeai/backend/model_manager/configs/controlnet.py`:

```python title="invokeai/backend/model_manager/configs/controlnet.py"
@ModelConfigFactory.register
...
**Backend Config** — `invokeai/backend/model_manager/configs/lora.py`:

```python title="invokeai/backend/model_manager/configs/lora.py"
@ModelConfigFactory.register
...
**Frontend Component** — `invokeai/frontend/web/src/features/parameters/components/NewModelScheduler.tsx`:

```typescript title="invokeai/frontend/web/src/features/parameters/components/NewModelScheduler.tsx"
...
:::tip[Checklist: Optional Features]{icon="approve-check"}
**ControlNet:**
...
:::note[What you no longer touch]
`app/util/step_callback.py` and `app/api/dependencies.py` used to need an edit per architecture and
are now derived from the registry. `starter_models.py` is a package rather than one 2400-line file.
The frontend column is unchanged — `webv2` has no OpenAPI tooling, so nothing there is generated.
:::
