...
- Windows on ARM64 with an NVIDIA RTX Spark GPU is supported as a **Developer Preview**, tracking NVIDIA's own preview status for the platform. See [Windows on ARM64](#windows-on-arm64-rtx-spark) below for the requirements and the feature differences.
...
| FLUX.2 Klein 9B | 1024x1024 | Nvidia 30xx+ | 12GB | 32GB | int8 9.5GB on any supported GPU; FP8 needs a 40xx **and** `fp8_compute` (off by default) to stay that small, else ~18GB. The Qwen3 8B encoder is 15GB and streams on a 12GB card |
| Z-Image Turbo | 1024x1024 | Nvidia 20xx+ | 8GB | 16GB | Q4_K / NVFP4 8GB; Q8/BF16 16GB+ |
...
#### Windows on ARM64 (RTX Spark)

Requirements:

- NVIDIA's RTX Spark Developer Driver, 616.41 or later, and Windows 11 ARM64. NVIDIA's [RTX Spark developer page] has the driver and the porting guide; the CUDA Toolkit itself is not needed, the torch wheel bundles its CUDA 13.4 libraries.
- Python **3.12**, as a native ARM64 build. An x86_64 Python runs under emulation but cannot load the ARM64 torch wheel. `uv` installs an x86_64 interpreter on ARM64 Windows unless asked for the native one: request `cpython-3.12-windows-aarch64-none` (see the [manual install](../manual) guide).
- torch comes from NVIDIA's index (`https://pypi.nvidia.com/nvtorch_oot/`), not from PyTorch's, and the torch wheel is 1.85 GB. `uv sync` from a source checkout handles this automatically; a manual `uv pip install` needs the `--index` and `--find-links` options from the manual install guide. The Invoke Launcher does not support Windows ARM64 yet; use the manual install until a launcher release adds it.
- An `ffmpeg` on `PATH` (or the `IMAGEIO_FFMPEG_EXE` environment variable) for the video nodes and video thumbnails. On other platforms the `imageio-ffmpeg` package bundles one; it has no ARM64 build.

Differences from Windows x64, all because the underlying package has no Windows ARM64 build:

- The MediaPipe face detection preprocessor and the Face Tools nodes (Face Identifier, Face Mask, Face Off) report an error when run.
- The image map projects with PCA instead of UMAP, so it is less structured.
- `xformers` is not available; torch's own attention kernels are used.
- `.woff2` fonts cannot be read: the font list skips them and uploads are rejected. `.ttf`, `.otf` and `.woff` work.
- PyPatchMatch has no prebuilt library for Windows ARM64, so the `patchmatch` infill method is unavailable.

GPU generation on RTX Spark has not yet been validated by the Invoke team on hardware; installation and the CPU code paths are exercised in CI on Windows ARM64 runners.
...
:::note[Wide-head attention runs on the math kernel]
  ...
  head. The math kernel materializes the attention matrix, so large untiled VAE decodes reserve
  more VRAM (about 4 GB at 1024px, 21 GB at 1536px) and may push other models out of the cache;
  tiled decoding keeps that bounded. Export `INVOKE_ROCM_FUSED_SDPA_MAX_HEAD_DIM` to raise the
  threshold on a build whose fused kernels are known to be correct.
:::
...
[RTX Spark developer page]: https://developer.nvidia.com/topics/ai/local-ai/port-apps
