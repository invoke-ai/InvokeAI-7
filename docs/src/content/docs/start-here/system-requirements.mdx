...
- torch comes from NVIDIA's index (`https://pypi.nvidia.com/nvtorch_oot/`), not from PyTorch's, and the torch wheel is 1.85 GB. The launcher and `uv sync` handle this automatically; a manual `uv pip install` needs the `--index` and `--find-links` options from the manual install guide.
...
- `.woff2` fonts are skipped by the font list; `.ttf`, `.otf` and `.woff` work.
- PyPatchMatch does not install; the `patchmatch` infill method is hidden.
- ONNX models (the DW OpenPose preprocessor) run on the CPU.
