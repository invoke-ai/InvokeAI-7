...
To always decode untiled unless you ask for tiling, turn it off. A decode too large for the card then runs out of memory: the FLUX.1, Z-Image and Anima decodes retry in tiles, the others fail, and on Windows the driver may move the decode into system memory and run it very slowly instead.
