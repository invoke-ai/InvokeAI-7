...
## No ffmpeg exe could be found (Windows on ARM64)

Video nodes and video thumbnails use `ffmpeg`. On every other platform the `imageio-ffmpeg` package bundles the binary; it has no Windows ARM64 build, so on ARM64 Invoke uses the `ffmpeg` on your `PATH`. Install one (an x64 build runs under Windows' emulation) or point the `IMAGEIO_FFMPEG_EXE` environment variable at an `ffmpeg.exe`, then restart Invoke.
