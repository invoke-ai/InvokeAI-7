...
description: Generate short videos — with audio on MiniMax H3 and LTX-2 — from the Video panel. Covers the panel's controls, the Wan 2.2, MiniMax H3 and LTX-2 model families, and how to make longer videos.
lastUpdated: 2026-09-19
---

import { Card, CardGrid, Steps } from '@astrojs/starlight/components';

InvokeAI generates short MP4 clips from a text prompt, one or two images, an existing video — or combinations of those — using three local model families:
...
* **LTX-2** — Lightricks' 22B dual-stream transformer: text-to-video and image-to-video, each with a jointly generated stereo soundtrack, in a **Dev** build you steer with guidance and a **Distilled** build that renders in eight fixed steps.
...
1. **Install a video model.** Open the **Model Manager** and install a starter bundle: **Wan 2.2 Text-to-Video**, **Wan 2.2 Image-to-Video**, **MiniMax H3**, or **LTX-2**. See [the models](#the-models) below for what each contains and what fits your hardware.
...
* **LTX-2** — offered on the Dev build, where it is pre-filled with the release's own default list of artifact terms, and hidden on the Distilled build, which runs no guidance. Turning CFG down to 1 on Dev drops it from the graph, and the 12B prompt encode it would have cost.
...
A first frame is anchored on both passes, encoded once at each canvas. The refine pass re-noises every frame including the first, so an anchor that was only applied to the base pass would not survive into the output.

#### Installing LTX-2

The **LTX-2** starter bundle installs the working set:
...
Extension is available on Wan and MiniMax H3. LTX-2 has no extend mode yet, so a longer LTX-2 clip means asking for more frames and accepting the drift.
