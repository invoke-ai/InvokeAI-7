...
are not supported as user-facing workflow shapes. The backend has a bounded
two-branch execution-planner case for an internal body-mediated fan-in, but
that implementation detail does not expand the supported workflow contract.
