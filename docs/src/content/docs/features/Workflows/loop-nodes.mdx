...
- **Output** adds one value to `For.output_collection` for the current iteration. Null (`None`) values are retained, so
  collection positions continue to match iteration order.
...
An empty input to `Concatenate` is harmless. An empty input to `Zip` is valid when both inputs are empty. An empty
...
- Use **Iterate** when a normal workflow needs to expand a collection into separate executions and **Collect** when
  those executions should be gathered again.
- Use **For** when the repeated work has a clear beginning and end, needs loop state, may stop early, or should expose
  a final collection and final state.
