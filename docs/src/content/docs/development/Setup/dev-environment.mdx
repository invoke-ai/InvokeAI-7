...
<Steps>
  ...
  5. Follow the [manual install](/start-here/manual/) guide, with some modifications to the install command:
      ...
      :::note[Windows on ARM64 (RTX Spark)]
        Use `--python cpython-3.12-windows-aarch64-none` (uv otherwise installs an x86_64 interpreter) and omit `--extra xformers`. Whichever GPU extra you pass, the lockfile installs NVIDIA's torch 2.14.0+cu134 build and Invoke's self-built OpenCV, blake3 and tiktoken wheels; no `--index` options are needed because `uv sync` reads them from `pyproject.toml`. Put an `ffmpeg` on `PATH` for the video tests.
      :::
