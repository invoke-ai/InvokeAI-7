...
### Importing InvokeAI APIs

Use `invokeai.invocation_api` for common invocation classes, fields, and
decorators:

```python
from invokeai.invocation_api import BaseInvocation, InputField, OutputField, invocation
```

Some built-in invocation modules live in subpackages. Import them from their
canonical paths, for example:
...
Legacy root-level import paths remain available through forwarding shims until
a separately approved cleanup. Use canonical paths for new node packs.
