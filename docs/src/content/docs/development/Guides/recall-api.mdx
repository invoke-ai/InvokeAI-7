...
:::tip
For the Video panel, see the [Video Recall API](/development/guides/video-recall-api/).
:::
...
4. **Broadcast** — a `recall_parameters_updated` event is emitted, once, to
   the calling user's room and the administrators' room. webv2 ignores events
   addressed to another user; the legacy frontend applies them, so an
   administrator using it also receives other users' recalls.
5. **Frontend update** — the calling user's connected clients
   apply the update to their generation settings, so UI fields, LoRAs,
   reference images and (in the legacy frontend) control layers populate
   immediately. See [Frontend behavior](#frontend-behavior) for what each
   frontend applies.
...
Parameter updates emit a `recall_parameters_updated` event to the calling
user's room and the administrators' room. The calling user's connected frontend
clients automatically:

1. Apply standard parameters (prompts, steps, dimensions, etc.).
2. Load and add LoRAs to the LoRA list.
3. Apply control-layer configurations (legacy frontend only).
...
The update lands in the **Generate** panel of the project that was active when
the event arrived, and shows a notification summarizing how many fields were
applied. If that project is still on screen, the Generate widget is brought to the
front the same way the [Video Recall API](/development/guides/video-recall-api/#frontend-behavior)
brings the Video widget forward: its tab is selected, a floating window is raised
(and unrolled if shaded),
and nothing moves while the user is typing. Parameters the Generate panel has no home for, and entries that could
not be resolved, are left out and named in a second notification so the caller
can tell what did not land. The editor must be open: events that arrive while
the user is on the Launchpad are dropped even though the request returns 200.
In multi-user mode only the requesting user's own sessions apply the update;
administrators' sessions ignore other users' recalls.

| Parameter                                                                                              | webv2 behavior                                                                                                                          |
| ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `positive_prompt`, `negative_prompt`, `seed`, `steps`, `cfg_scale`, `cfg_rescale_multiplier`, `scheduler`, `width`, `height`, `clip_skip`, `seamless_x`, `seamless_y` | Applied. A recalled `seed` also turns off seed randomization; `clip_skip` applies only to model families that expose it (SD 1.x / 2.x). |
| `model`                                                                                                | Switches the Generate model. Changing model applies that model's defaults (steps, CFG, CFG rescale, scheduler, size, VAE precision) and runs the same compatibility pass as the model selector (incompatible LoRAs, VAE and component selections are cleared; reference images are retargeted) before the explicit values in the same request are applied. |
| `vae_model`                                                                                            | Matched by key or name and applied when compatible with the effective model.                                                             |
| `guidance`                                                                                             | Applied as the CFG/guidance value for FLUX, FLUX.2, Qwen Image and Z-Image models when `cfg_scale` is not also given; reported as not applied otherwise. |
| `loras`                                                                                                | Replaces the LoRA list. Entries that are not installed or not compatible with the effective model are dropped and reported.              |
| `ip_adapters`, `reference_images`                                                                      | Feed the reference image list, retargeted to the effective model's reference-image type. Files no longer in the gallery are dropped and reported; if none of the recalled images exist the project keeps its current list. `append=true` adds behind the existing entries without re-checking them; the model's reference-image limit is enforced (newest entries are dropped first). |
| `control_layers`                                                                                       | **Not applied.** Control layers live on the Canvas in webv2, not in the Generate panel.                                                  |
| `denoise_strength`, `refiner_*`                                                                        | **Not applied.** The Generate panel has no image-to-image strength or SDXL refiner settings.                                              |

In `strict` mode, omitted scalar parameters reset to the effective model's
defaults (empty prompts, randomized seed, default steps/CFG/scheduler/size),
and omitted lists clear. An omitted `model` keeps the current model.

Events are applied in arrival order; an `append` request that overlaps a
still-loading replace sees the replaced list. The handler is loaded on the
first event of a session; if that load fails, the event is dropped, an error is
recorded in the notification center, and the next event retries the load.

### Legacy web

The legacy frontend applies prompts, steps, CFG, size, seed, model, LoRAs,
control layers and reference images to its Redux store. It ignores `null`
scalar values, so `strict` mode only clears the list-typed parameters there.
...
webv2 reports each applied update, and any parameters it could not apply, in
the notification center; failures are recorded as errors there as well.

In the legacy frontend, set `localStorage.ROARR_FILTER = 'debug'` in the
browser to see all debug messages under the `events` namespace.
