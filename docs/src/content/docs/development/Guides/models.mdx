...
3. **Declare the Architecture**

   Every `BaseModelType` that is not a sentinel needs a module under
   `invokeai/backend/architectures/defs/`, named after the base value with `-` replaced by `_`. It
   is picked up automatically — there is no import list — but `registry.validate()` runs at boot and
   refuses to start when the file is missing or when it omits a required facet, naming the file.

   Default generation parameters live in this declaration, keyed by variant. `None` is the fallback
   for a model whose variant is not listed.

   ```python title="invokeai/backend/architectures/defs/newmodel.py"
   register(
       BaseModelType.NewModel,
       ...,
       DefaultSettingsFacet({None: MainModelDefaultSettings(steps=20, cfg_scale=7.0)}),
   )
   ```

   `python scripts/new_architecture.py --base newmodel --name NewModel --write` generates this file
   with every required facet present but deliberately wrong, so it fails at boot until each value is
   filled in. It also prints the files it cannot generate, derived from the tree on each run.
</Steps>
...
:::tip[Checklist: Taxonomy]{icon="approve-check"}
  ...
  - [ ] Create `architectures/defs/<base>.py` with a `register(...)` call — the app will not boot without it
...
owned by one: VAE nodes go in `invocations/vae/`, text encoders in `invocations/text_encoder/`, and
PiD decode/upscale in `invocations/pid/`. Generic nodes — noise, primitives, image ops, metadata —
stay at the top level.

Keep the architecture prefix on the filename (`newmodel_denoise.py`) even inside `newmodel/`. The
package is discovered automatically, but it needs an `__init__.py`: a directory without one is not
a package, and every node in it would simply not exist at runtime.
:::
...
:::tip[Checklist: Invocations]{icon="approve-check"}
  ...
  - [ ] Model loader invocation (`[newmodel]/[newmodel]_model_loader.py`)
  - [ ] Text encoder invocation (`text_encoder/[newmodel]_text_encoder.py`)
  - [ ] Denoise invocation (`[newmodel]/[newmodel]_denoise.py`)
  - [ ] Extend the standard `noise` invocation if the architecture supports external noise
  - [ ] VAE encode/decode invocations (`vae/[newmodel]_vae_encode.py`, `vae/[newmodel]_vae_decode.py`)
  - [ ] `__init__.py` in `invocations/[newmodel]/`, or none of its nodes are discovered
:::
...
### ControlNet Support
Requires backend configuration (`configs/controlnet.py`), a custom invocation (`[newmodel]/[newmodel]_controlnet.py`), and frontend graph integration (`addControlNets`).
...
### IP-Adapter
Requires a custom invocation for image prompting (`[newmodel]/[newmodel]_ip_adapter.py`) and frontend integration via `addIPAdapters`.
...
The catalogue is a package: define the models in your own module, then list them in the package's
`__init__.py`.
...
`STARTER_MODELS` is a curated order — it is what the install dialog shows, in the sequence someone
chose — so insert where the model belongs rather than appending.

```python title="invokeai/backend/model_manager/starter_models/__init__.py"
from invokeai.backend.model_manager.starter_models.newmodel import newmodel_main
...
- [ ] Define VAE/Encoder StarterModels if separate — shared ones live in `starter_models/common.py`
- [ ] Set dependencies correctly if required
- [ ] Import and place them in `STARTER_MODELS` in `starter_models/__init__.py`
:::
...
- invokeai
  ...
  - backend
    - architectures/defs
      - `[newmodel].py` the architecture's declaration; the app refuses to boot without it
    - model_manager
      - taxonomy.py
      - configs
        - main.py
      ...
      - starter_models
        - `[newmodel].py`
        - `__init__.py` import the models here and place them in `STARTER_MODELS`
    - `[newmodel]`
      - sampling_utils.py
      - denoise.py
